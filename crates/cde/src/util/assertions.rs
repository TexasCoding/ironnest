...
// DETERMINISM(ironnest): jagua compared active-hazard sets with `std::collections::HashSet`. These
// are debug-only assertions, but `HashSet` is banned workspace-wide (per-process random iteration).
// They are rewritten below to order-independent `Vec` containment — identical set semantics, no hash.
...
fn qt_nodes_match(qn1: Option<&QTNode>, qn2: Option<&QTNode>) -> bool {
    ...
    match (qn1, qn2) {
        (Some(qn1), Some(qn2)) => {
            ...
            //collect active hazards (order-independent comparison, no HashSet)
            let active_haz_1 = hv1.iter().map(hashable).collect_vec();
            let active_haz_2 = hv2.iter().map(hashable).collect_vec();

            let active_in_1_but_not_2 = active_haz_1
                .iter()
                .filter(|&h| !active_haz_2.contains(h))
                .collect_vec();
            let active_in_2_but_not_1 = active_haz_2
                .iter()
                .filter(|&h| !active_haz_1.contains(h))
                .collect_vec();
...
fn hazards_match<'a>(
    ...
    let chv1_active_hazards = chv1.map(|h| h.entity).collect_vec();
    let chv2_active_hazards = chv2.map(|h| h.entity).collect_vec();

    //set-equal (order/multiplicity-independent), without a HashSet
    let same = chv1_active_hazards
        .iter()
        .all(|h| chv2_active_hazards.contains(h))
        && chv2_active_hazards
            .iter()
            .all(|h| chv1_active_hazards.contains(h));
    if !same {
        error!("Hazard vecs don't match");
        return false;
    }
    true
}
