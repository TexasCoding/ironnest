...
impl Importer {
    ...
    pub fn new(
        ...
        simplify_tolerance: Option<Scalar>,
        min_item_separation: Option<Scalar>,
        narrow_concavity_cutoff: Option<(Scalar, Scalar)>,
    ) -> Importer {
        Importer {
...
pub fn eliminate_degenerate_vertices(points: &mut Vec<Point>) {
    ...
    for i in 0..n_points {
        ...
        let p_i = points[i];
        let p_j = points[j];
        if approx_eq!(Scalar, p_i.0, p_j.0) && approx_eq!(Scalar, p_i.1, p_j.1) {
