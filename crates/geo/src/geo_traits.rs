...
pub trait DistanceTo<T> {
    ...
    fn distance_to(&self, other: &T) -> Scalar;

    /// Squared version of [`DistanceTo::distance_to`]
    fn sq_distance_to(&self, other: &T) -> Scalar;
}
...
pub trait SeparationDistance<T>: DistanceTo<T> {
    ...
    fn separation_distance(&self, other: &T) -> (GeoPosition, Scalar);

    /// Squared version of [`SeparationDistance::separation_distance`]
    fn sq_separation_distance(&self, other: &T) -> (GeoPosition, Scalar);
}
