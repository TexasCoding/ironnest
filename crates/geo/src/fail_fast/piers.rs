...
use crate::geo_traits::{CollidesWith, DistanceTo, Transformable};
...
static CLIPPING_TRIM: Scalar = 0.999;
static ACTION_RADIUS_RATIO: Scalar = 0.10;
...
pub fn generate_piers(shape: &SPolygon, n: usize, poles: &[Circle]) -> Result<Vec<Edge>> {
    ...
    let forfeit_distance = Scalar::sqrt(bbox.width().powi(2) * bbox.height().powi(2));
...
fn generate_ray_transformations(
    ...
    //translations
    let dx = bbox.width() / rays_per_angle as Scalar;
    let translations = (0..rays_per_angle)
        .map(|i| bbox.x_min + dx * i as Scalar)
        .map(|x| Transformation::from_translation((x, 0.0)))
        .collect_vec();

    let angles = Array::linspace(0.0, Scalar::PI(), n_angles + 1).to_vec();
...
fn loss_function(
    new_ray: &Edge,
    point_grid: &[Point],
    min_distance_to_rays: &[Scalar],
    min_distance_to_poles: &[Scalar],
    radius_of_ray_influence: Scalar,
) -> Scalar {
    ...
    .map(|(p, min_distance_to_existing_ray, min_distance_to_pole)| {
        ...
        let min_distance_to_ray = Scalar::min(*min_distance_to_existing_ray, distance_to_new_ray);

        if min_distance_to_ray < radius_of_ray_influence {
            Scalar::min(*min_distance_to_pole, min_distance_to_ray)
        } else {
...
fn min_distances_to_rays(points: &[Point], rays: &[Edge], forfeit_distance: Scalar) -> Vec<Scalar> {
    points
        .iter()
        .map(|p| {
            rays.iter()
                .map(|r| r.distance_to(p))
                .fold(forfeit_distance, Scalar::min)
        })
        .collect_vec()
}
