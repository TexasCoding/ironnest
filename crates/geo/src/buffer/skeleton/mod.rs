...
use crate::buffer::priority_queue::PriorityQueue;
use crate::buffer::util::*;
use crate::buffer::vertex_queue::*;

// DETERMINISM(ironnest) / dependency hygiene: upstream geo-buffer used `geo::Winding` and
// `geo::Contains`, which drag in the full `geo` crate (→ rstar → atomic-polyfill). We reimplement
// the two operations it actually used directly on `geo-types` rings.
...
/// Signed-area (shoelace) winding of a closed ring. `None` if degenerate (< 3 points / zero area).
/// CCW ⇒ positive area (an outer ring); CW ⇒ negative (a hole). A duplicated closing vertex, if
/// present, contributes a zero-area term and is harmless.
fn winding_order(ls: &LineString) -> Option<WindingOrder> {
    let pts = &ls.0;
    if pts.len() < 3 {
        return None;
    }
...
/// Whether `poly`'s exterior ring contains the ring `ls` — checked by ray-cast point-in-polygon on
/// each of `ls`'s vertices. Sufficient for assigning a (CW) skeleton hole-ring to its enclosing
/// (CCW) outer ring, since the straight skeleton yields cleanly nested, non-crossing rings.
fn polygon_contains_linestring(poly: &Polygon, ls: &LineString) -> bool {
    let ext = poly.exterior();
    !ls.0.is_empty() && ls.0.iter().all(|&p| point_in_ring(ext, p))
}

/// Ray-casting point-in-polygon against a ring (a duplicated closing vertex is harmless).
fn point_in_ring(ring: &LineString, p: Coord) -> bool {
    let pts = &ring.0;
    if pts.len() < 3 {
        return false;
    }
...
impl Skeleton {
    pub(crate) fn apply_vertex_queue(
        ...
        for ls in &lsv {
            if winding_order(ls) == Some(WindingOrder::CounterClockwise) {
                let p1: Polygon = Polygon::new(ls.clone(), vec![]);
                res.push(p1);
            }
        }
        for ls in &lsv {
            if winding_order(ls) == Some(WindingOrder::Clockwise) {
                for e in &mut res {
                    if polygon_contains_linestring(e, ls) {
