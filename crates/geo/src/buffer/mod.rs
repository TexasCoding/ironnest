// VENDORED — geo-buffer 0.2.0 (https://crates.io/crates/geo-buffer), Apache-2.0,
// © Teneleven Corp / Inseop Youn. See buffer/NOTICE for the license. This is a straight-skeleton
// polygon offsetter; ironnest vendors it (instead of depending on the crate) for two reasons:
//   1. DETERMINISM(ironnest): the upstream `util::ray::rotate_by` calls std `f64::sin`/`cos`, which
//      route through the platform C libm and differ in the last ULPs across glibc / MSVCRT / Darwin.
//      We swap those two lines to the pure-Rust `libm` crate so the offset is byte-identical on every
//      target (this is the fix for docs/00 risk #2 — the nonzero-min-separation path).
//   2. Dependency hygiene: the upstream crate pulls the full `geo` crate (→ rstar → heapless →
//      the unmaintained atomic-polyfill, RUSTSEC-2023-0089). We re-implement the two `geo` traits it
//      used (`Winding`, `Contains`) on top of `geo-types` so this code needs only `geo-types`.
// Apart from those edits the algorithm is upstream's; module-level lint allows below exempt the
// vendored style from our pedantic gate while keeping the determinism lints (disallowed_methods/types)
// active so any reintroduced std transcendental is caught.
#![allow(
    clippy::pedantic,
    clippy::nursery,
    clippy::complexity,
...
//! The (vendored) `geo-buffer` straight-skeleton offsetter — buffer (inflate / deflate) polygons.
//!
