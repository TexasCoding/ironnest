...
impl Ray {
    ...
    pub fn rotate_by(&self, angle: f64) -> Self {
        // DETERMINISM(ironnest): pure-Rust `libm` trig (byte-identical across platforms) in place of
        // std `f64::cos`/`sin` — the lone non-portable spot in the vendored offsetter. See buffer/mod.rs.
        let (sin, cos) = (libm::sin(angle), libm::cos(angle));
        let nx = self.angle.0 * cos - self.angle.1 * sin;
        let ny = self.angle.0 * sin + self.angle.1 * cos;
        Self {
            origin: self.origin,
            angle: (nx, ny).into(),
        }
