...
use crate::convex_hull::convex_hull_from_points;
use crate::fail_fast::{SPSurrogate, SPSurrogateConfig, compute_pole};
use crate::fpa::FPA;
...
impl SPolygon {
    ...
    #[must_use]
    pub fn calculate_diameter(points: Vec<Point>) -> Scalar {
    ...
    pub fn generate_bounding_box(points: &[Point]) -> Rect {
        let (mut x_min, mut y_min) = (Scalar::MAX, Scalar::MAX);
        let (mut x_max, mut y_max) = (Scalar::MIN, Scalar::MIN);
    ...
    #[must_use]
    pub fn calculate_area(points: &[Point]) -> Scalar {
        let mut sigma: Scalar = 0.0;
    ...
    pub fn calculate_poi(points: &[Point], diameter: Scalar) -> Result<Circle> {
        ...
        let dummy_sp = {
            ...
            let dummy_poi = Circle::try_new(Point(Scalar::MAX, Scalar::MAX), Scalar::MAX).unwrap();
