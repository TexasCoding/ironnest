...
use crate::geo_enums::GeoPosition;
...
impl Circle {
    pub fn try_new(center: Point, radius: Scalar) -> Result<Self> {
        ensure!(
...
impl CollidesWith<Rect> for Circle {
    ...
    fn collides_with(&self, rect: &Rect) -> bool {
        ...
        let nearest_x = Scalar::max(rect.x_min, Scalar::min(c_x, rect.x_max));
        let nearest_y = Scalar::max(rect.y_min, Scalar::min(c_y, rect.y_max));
...
impl SeparationDistance<Point> for Circle {
    fn separation_distance(&self, point: &Point) -> (GeoPosition, Scalar) {
        let Point(x, y) = point;
        let Point(cx, cy) = self.center;
        let d_center = Scalar::sqrt((x - cx).powi(2) + (y - cy).powi(2));
...
impl DistanceTo<Circle> for Circle {
    fn distance_to(&self, other: &Circle) -> Scalar {
    ...
    fn sq_distance_to(&self, other: &Circle) -> Scalar {
        self.distance_to(other).powi(2)
    }
}

impl SeparationDistance<Circle> for Circle {
    fn separation_distance(&self, other: &Circle) -> (GeoPosition, Scalar) {
    ...
    fn sq_separation_distance(&self, other: &Circle) -> (GeoPosition, Scalar) {
...
impl DistanceTo<Edge> for Circle {
    fn distance_to(&self, e: &Edge) -> Scalar {
    ...
    fn sq_distance_to(&self, e: &Edge) -> Scalar {
        self.distance_to(e).powi(2)
    }
}

impl SeparationDistance<Edge> for Circle {
    fn separation_distance(&self, e: &Edge) -> (GeoPosition, Scalar) {
    ...
    fn sq_separation_distance(&self, e: &Edge) -> (GeoPosition, Scalar) {
